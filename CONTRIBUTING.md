...
`pnpm images` is the one command that is not a gate. It redraws every picture the
instance serves into `apps/web/public`, from the tokens, the mark and the font
files the product already ships: the two share cards, `favicon.ico`, and the
raster icon Google asks for. Its output is committed. Run it when the design
moves, and edit `apps/web/social.ts` rather than the pictures, because the words
on a card and the words in the head come from that one table.
