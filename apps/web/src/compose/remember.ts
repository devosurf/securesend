...
/**
 * Puts one secret at the front of this browser's memory, newest first, which is
 * the only order a device-local list can honestly have: there is no name to sort
 * by and no sort control to offer.
 *
 * Rows a week past their expiry are dropped on the way through. That is the
 * forgetting the product tells senders about, and doing it on write is what keeps
 * a browser that sends every day from carrying a year of dead ids.
 */
export function remember(secret: SentSecret, kept: Kept, now = Date.now()) {
  const rest = read(kept).filter(
    (held) => held.id !== secret.id && stillWorthKeeping(held, now)
  );

  try {
    kept.setItem(KEY, JSON.stringify([secret, ...rest]));
  } catch {
